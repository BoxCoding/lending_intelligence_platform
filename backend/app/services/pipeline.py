...
def process_aa_payload(payload: AAPayload) -> CustomerProfile:
    ...
    store.put("customers", payload.customer_id, {
        "customer_id": payload.customer_id,
        "name": payload.name,
    ...
    store.put("lead_scores", payload.customer_id, doc["lead"] | {"customer_id": payload.customer_id})
    ...
    logger.info(
        ...
        payload.customer_id, income.monthly_income, lead.tier, lead.score, risk.risk_grade,
    )
    return profile

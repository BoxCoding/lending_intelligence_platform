    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted">
            Pre-qualified lending leads from Account Aggregator intelligence
          </p>
        </div>
        <span className="text-xs text-muted">
          Models:{" "}
          {Object.entries(data.models)
            .map(([k, v]) => `${k} ${v ? "✓ML" : "rules"}`)
            .join(" · ")}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Stat
          ...
          value={kpis.predicted_conversions}
          accent="text-success"
        />
        <Stat
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Section title="Lead Funnel">
          <LeadFunnel data={charts.lead_funnel ?? []} />
        </Section>
      ...
      <Section
        title={`Lead Queue (${leads.length})`}
        right={<span className="text-xs text-muted">sorted by score</span>}
      >
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <caption className="sr-only">
              Pre-qualified leads ranked by score, with tier, estimated income, risk grade, intent
              score, top recommended product, and conversion probability
            </caption>
            ...
            <tbody>
              {leads.slice(0, 50).map((l) => (
                ...
                >
                  ...
                  <td className="pr-4">
                    <span className={`badge ${tierColor(l.tier)}`}>{l.tier}</span>
                  </td>

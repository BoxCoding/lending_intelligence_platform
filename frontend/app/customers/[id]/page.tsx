  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className={`badge ${tierColor(p.lead.tier)}`}>
          {p.lead.tier} · {p.lead.score}
        </span>
        <span className="badge bg-surface text-muted">Risk {p.risk.risk_grade}</span>
        <Link
          href={`/chat?customer=${p.customer_id}`}
          className="ml-auto rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary/85"
        >
          Ask AI about this customer
        </Link>
      </div>

      <p className="card text-sm text-slate-700">{p.recommendation.summary}</p>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <Stat
          ...
          value={`${p.recommendation.financial_health_score}/100`}
          accent="text-success"
        />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Section
          ...
          right={
            <span className="text-xs text-muted">
              confidence {Math.round(p.income.confidence * 100)}%
            </span>
          }
        >
          <div className="space-y-3 text-sm">
            ...
            <div>
              <p className="mb-1 flex justify-between text-muted">
                ...
                <span>{Math.round(p.income.cash_flow_stability * 100)}%</span>
              </p>
        ...
        <Section
          ...
          right={<span className="text-xs text-muted">score {p.intent.intent_score}/100</span>}
        >
        ...
        <Section
          ...
          right={
            <span className="text-xs text-muted">
              PD {(p.risk.probability_of_default * 100).toFixed(1)}%
            </span>
          }
        >
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-muted">Financial stability</span>
              <span>{p.risk.financial_stability}/100</span>
            </div>
            <Meter value={p.risk.financial_stability} color="bg-success" />
            <div className="flex justify-between">
              <span className="text-muted">Behaviour stability</span>
              <span>{p.risk.behavior_stability}/100</span>
            </div>
            <Meter value={p.risk.behavior_stability} color="bg-cold" />
            <div className="flex justify-between border-b border-border/50 pb-2">
              <span className="text-muted">Liquidity risk</span>
              <span>{p.risk.liquidity_risk}</span>
            </div>
            <div>
              ...
              {p.risk.fraud_indicators.length === 0 ? (
                ...
                <ul className="list-inside list-disc text-hot">
                  {p.risk.fraud_indicators.map((f) => (
                    <li key={f}>{f}</li>
                  ))}
                </ul>
              )}
      ...
      <Section title="Recommended Offers">
        <div className="grid gap-4 lg:grid-cols-3">
          {p.recommendation.offers.length === 0 && (
            <p className="text-muted">No qualifying offers.</p>
          )}
          {p.recommendation.offers.map((o) => (
            <div key={o.product} className="rounded-lg border border-border bg-surface p-4">
              <div className="flex items-center justify-between">
                ...
                <span className="badge bg-primary/15 text-primary">#{o.priority}</span>
              </div>
              <p className="mt-2 text-2xl font-bold">{inr(o.eligible_amount)}</p>
              <p className="text-xs text-muted">
                {o.interest_rate_min}–{o.interest_rate_max}% · {o.tenure_months} months · EMI{" "}
                {inr(o.monthly_emi)}
              </p>
      ...
      {explain && (
        <Section
          title="Why (SHAP) — Risk Model Drivers"
          right={
            <span className="text-xs text-muted">
              green lowers approval risk concern · red raises it
            </span>
          }
        >
    ...
    <Section title="What-if Repayment Simulator">
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <button
          ...
          className="self-end rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary/85 disabled:opacity-50"
        >
      ...
      {mutation.data && (
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
          <div>
            ...
            <p className="text-lg font-bold">{inr(mutation.data.monthly_emi)}</p>
          </div>
          <div>
            <p className="text-muted">Projected FOIR</p>
            <p className="text-lg font-bold">{Math.round(mutation.data.projected_foir * 100)}%</p>
          </div>
          <div>
            <p className="text-muted">Disposable after EMI</p>
            <p className="text-lg font-bold">{inr(mutation.data.disposable_after_emi)}</p>
          </div>
          <div>
            <p className="text-muted">Verdict</p>
            <p className={`text-lg font-bold ${verdictColor(mutation.data.verdict)}`}>
